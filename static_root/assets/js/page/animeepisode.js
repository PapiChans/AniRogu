...
var MarkAsCompleted_Button = document.getElementById('MarkAsCompleted_Button');
...
function AnimeMarkAsCompleted(anime_Id){

    MarkAsCompleted_Button.disabled = true;

    Swal.fire({
        title: "Are your Sure?",
        text: "All episodes are mark as completed, Continue?",
        icon: "question",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
            ...
            // Construct the URL with query parameters
            const url = `/backend/backend/AnimeMarkAsCompleted/${encodeURIComponent(anime_Id)}`;

            // Redirect to the constructed URL
            window.location.href = url;

        } else {
            MarkAsCompleted_Button.disabled = false;
        }
    });
}